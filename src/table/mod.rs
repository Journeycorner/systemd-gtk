...
use gtk::{Align, ColumnView, ColumnViewColumn, CustomSorter, Label, ListItem, ListItemFactory, Ordering, SignalListItemFactory};
...
pub(crate) fn setup_columns(column_view: &ColumnView) {
    ...
    name_factory.connect_bind(move |_, list_item|
        build_label(list_item, |unit_object| unit_object.unit_file().to_string(), 30));
    ...
    load_factory.connect_bind(move |_, list_item|
        build_label(list_item, |unit_object| unit_object.load().unwrap_or_default().to_string(), 1_000));
    ...
    active_factory.connect_bind(move |_, list_item|
        build_label(list_item, |unit_object| unit_object.active().unwrap_or_default().to_string(), 1_000));
    ...
    description_factory.connect_bind(move |_, list_item|
        build_label(list_item, |unit_object| unit_object.description().unwrap_or_default().to_string(), 1_000));
...
fn with_expand(unit_name: &str, name_factory: SignalListItemFactory) -> ColumnViewColumn {
    ...
    let sorter = CustomSorter::new(|one, two| {
        Ordering::Equal
    });
